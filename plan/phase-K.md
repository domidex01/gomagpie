...
2. `go doc github.com/domidex01/magpie` resolves from a *second* module: in `magpie-desktop`, `go mod tidy && go build ./...` succeeds with `require github.com/domidex01/magpie v0.1.0` (no replace).
...
**Sanity check:** `go list -m github.com/domidex01/magpie@v0.1.0` resolves from outside the repo.

### Task K.2 — Private repo bootstrap (1.5h)

**Depends on:** K.1.

- Create private GitHub repo `domidex01/magpie-desktop` (closed). Locally: `magpie-desktop/` with `go.mod` (`module github.com/domidex01/magpie-desktop`, `require github.com/domidex01/magpie v0.1.0`, plus `replace github.com/domidex01/magpie => ../magpie` during K), `main.go`, `internal/service/`, `frontend/`, `Taskfile.yml`, `.gitignore` (`bin/`, `frontend/bindings/`, `frontend/node_modules/`).
...
**Sanity check:** `wails3 generate bindings -ts` emits `frontend/bindings/github.com/domidex01/magpie-desktop/internal/service/*.ts` without warnings; a temporary button calling `ScrapeService.Scrape("https://example.com", …)` returns markdown.
...
├── go.mod                      # module github.com/domidex01/magpie
...
├── go.mod                      # requires github.com/domidex01/magpie v0.1.0 (+ replace ../magpie in dev)
...
- [ ] Public repo: module renamed to `github.com/domidex01/magpie`; all gates green; `git diff testdata/` shows only the buildmod replace line; `LICENSE` = MIT
- [ ] `store.ListRuns` exists with a test; `GetRun`/`CrawlStats` behavior unchanged
- [ ] `v0.1.0` tagged and pushed; `go list -m github.com/domidex01/magpie@v0.1.0` resolves
...
import { ScrapeService } from '../bindings/github.com/domidex01/magpie-desktop/internal/service';
