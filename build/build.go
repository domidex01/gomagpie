...
import (
{{range .}}	_ "{{.}}"
{{end}}	"github.com/motherlodelab/magpie/cli"
	"os"
)
...
func Build(ctx context.Context, o BuildOptions) error {
	...
	if err := run("go", "mod", "edit", "-require=github.com/motherlodelab/magpie@v0.0.0", "-replace=github.com/motherlodelab/magpie="+root); err != nil {
		return err
	}
...
func magpieRoot(ctx context.Context) (string, error) {
	...
	cmd := exec.CommandContext(ctx, "go", "list", "-m", "-f", "{{.Dir}}", "github.com/motherlodelab/magpie")
